 ...
 * The SDK handles stepTransaction, includedSteps, approvals, and confirmation
 * automatically via a viem WalletClient backed by the delegated Dynamic wallet.
 */
...
/**
 * Lazily initialise the LI.FI SDK with a viem WalletClient backed by the
 * delegated Dynamic wallet. Called once before the first route execution.
 */
  ...
  const buildAccount = () =>
    toAccount({
      ...
      async signMessage({ message }) {
        const msg =
          typeof message === "string"
            ? message
            : Buffer.from((message as { raw: Uint8Array }).raw).toString(
                "utf8"
              );
      ...
      async signTypedData({ domain, types, primaryType, message }) {
        // Build EIP712Domain only for fields actually present in the domain
        // (omitting absent fields like verifyingContract avoids viem throwing
      ...
      EVM({
        getWalletClient: async () =>
          createWalletClient({
            ...
            chain: getChainById(137), // default; switchChain handles the rest
...
/**
 * High-level helper: find routes to get USDC on Polygon from any token.
 */
...
/**
 * Execute a LI.FI route using the SDK's executeRoute().
 * The SDK manages approvals, step transactions, and confirmation.
 */
  ...
  try {
    await sdkExecuteRoute(route, {
      ...
      updateTransactionRequestHook: async (txRequest) => {
        // 2.5× gas buffer to prevent reverts on complex swaps
