...
- **Written**: `POST /api/webhooks` triggers Dynamic's `wallet.delegation.created` event, which calls `storeDelegation()`.
...
- **Deleted**: `wallet.delegation.revoked` webhook event calls `deleteDelegation(userId)`.
