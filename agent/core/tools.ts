...
// Supported chain names per the Dynamic API
  ...
  async ({ chainName, networkId, includePrices }) => {
    try {
      if (!agentWallet) {
        return JSON.stringify({
          success: false,
          error: "No agent wallet loaded. Set delegation credentials in .env.",
        });
      }
      ...
      if (!environmentId || !userJwt) {
        return JSON.stringify({
          success: false,
          error: "DYNAMIC_ENVIRONMENT_ID or DYNAMIC_USER_JWT not set",
        });
      }

      const chain = chainName?.toUpperCase() ?? "EVM";

      // Decode JWT payload to extract session_public_key (no verification needed)
      ...
      // The Dynamic EVM balance API requires a networkId — querying without one returns [].
      // When no networkId is specified, fan out across popular EVM networks.
